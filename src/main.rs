...
use optimize::DifferentialEvolution;
...
struct Himmelblau {
    ...
    //cost: f64,
    //params: Vec<f64>
}//{upper_bound: Vec<f64>, lower_bound: Vec<f64>, rng: Arc<Mutex<XorShiftRng>>}

impl Himmelblau {
    fn cost_fn(&self, v: &[f64]) -> f64 {
        (v[0].powi(2) + v[1]-11.).powi(2) + (v[0] + v[1].powi(2) -7.).powi(2)
    }
}

//impl Evolution for Himmelblau {
//    fn evo(&self) -> Vec<f64>;
//}
...
impl DifferentialEvolution for Himmelblau {
    fn get_bounds(&self) -> &Vec<(f64, f64)> {
        &self.bounds
    }
...
trait Evo {
    fn evo<F>(bounds: Vec<(f64, f64)>, obj_func: F) -> (Vec<f64>, f64) where F: Fn(&[f64]) -> f64 {
        let pop_size: usize = 25*bounds.len();
        let f: f64 = 0.8;
        let cr: f64 = 0.9;
        let max_iter: usize = 1000;
...
fn main() {
    ...
    let initial_min_max = vec![(-5., 5.); 2];
    let mut testeroo = Himmelblau{bounds: initial_min_max, cost: 0.0, params: vec![0.0]};
    let answer = testeroo.get_cost(&[3.01, 2.]);
    println!("{:?}", answer);
    let start = Instant::now();
    let (mut opt_vec, mut opt_val) = testeroo.minimum();
    let end = start.elapsed();
    println!("{:?} seconds", end.as_seconds_f64());
    //let answer = testeroo.cost_fn(&[3.01, 2.]);
    println!("{:?}, {:?}", opt_vec, opt_val);
    //testeroo.minimum();
    println!("{:?}", testeroo);
}
